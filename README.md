...
The MVOR dataset is hosted on the [unistra S3 server](https://s3.unistra.fr/camma_public/datasets/mvor/camma_mvor_dataset.zip). Please download the dataset and install the dependencies using the commnds given below.
...
[cameras_info] : Contains the intrinsics (focallength, principalpoint, distortion) and
...
[licenses] : Contains the link to license of Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)
[dataset_info] : Contains the camera id to camera number mapping and also day id to day number mapping.
[categories] : Contains the 10 upperbody skeleton information
```
...
The MVOR dataset is publicly released under the Creative Commons licence [CC-BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/). This implies that:
